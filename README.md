...
runtime:
  seed: 42
  require_exact_window_coverage: true
  overwrite: false
```
...
```bash
uv run ivs-forecast verify-data --config configs/experiments/spx_full_available_2004_2021q2.yaml
```

Build the immutable preprocessing artifacts:

```bash
uv run ivs-forecast build-data --config configs/experiments/spx_full_available_2004_2021q2.yaml
```

Run the full experiment:

```bash
uv run ivs-forecast run --config configs/experiments/spx_full_available_2004_2021q2.yaml
```
